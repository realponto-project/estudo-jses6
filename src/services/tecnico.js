...
export const getAllTecnico = async (query) => {
  ...
  await axios.get(`${BACKEND_URL}/api/technician/getAllTechnician`, { headers: headers, params: { query } }).then(
    resp => {
      response = resp
    }
  ).catch((error) => {
...
export const newCarro = async (values) => {
  ...
  await axios.post(`${BACKEND_URL}/api/car`, values, { headers: headers }).then(
    resp => {
      response = resp
    }
  ).catch((error) => {
